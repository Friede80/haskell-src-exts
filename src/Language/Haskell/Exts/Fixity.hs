...
class AppFixity ast where
  ...
  applyFixities :: (Monad m, MonadFail m) => [Fixity]      -- ^ The fixities to account for.
...
appFixDecls :: (Monad m, MonadFail m) => Maybe (ModuleName SrcSpanInfo) -> [Fixity] -> [Decl SrcSpanInfo] -> m [Decl SrcSpanInfo]
...
leafFix :: (Monad m, MonadFail m) => [Fixity] -> Exp SrcSpanInfo -> m (Exp SrcSpanInfo)
...
leafFixP :: (Monad m, MonadFail m) => [Fixity] -> Pat SrcSpanInfo -> m (Pat SrcSpanInfo)
